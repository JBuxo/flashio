...
export default function CompletionOverlay() {
  ...
  const answers = { correct: 20, wrong: 0 };

  //   XP earned based on right and wrong
  const xp =
    (answers.correct / (answers.correct + answers.wrong)) * pack!.reward_xp;

  //   Shards Earned based on percentage of right and wrong
  const shards =
    (answers.correct / (answers.correct + answers.wrong)) *
    pack!.reward_clevershard;
  return (
    <motion.div
          ...
          <div className="border-3 border-black h-full flex-1 my-4 p-4 text-xl">
            <ul className="space-y-4">
              <li className="flex items-center">
                ...
                Correct x {answers.correct}{" "}
              </li>
              <li className="flex items-center">
                <SquareXIcon className="mr-2 fill-red-500 text-red-800" /> Wrong
                x {answers.wrong}{" "}
              </li>
              <li className="flex items-center">
                <Image
